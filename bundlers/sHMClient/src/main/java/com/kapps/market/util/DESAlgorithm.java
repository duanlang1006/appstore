...
public class DESAlgorithm {
	...
	private final static byte[] PC1_Table = { 57, 49, 41, 33, 25, 17, 9, 1, 58, 50, 42, 34, 26, 18, 10, 2, 59, 51, 43,
			35, 27, 19, 11, 3, 60, 52, 44, 36, 63, 55, 47, 39, 31, 23, 15, 7, 62, 54, 46, 38, 30, 22, 14, 6, 61, 53,
	...
	private final static byte[] PC2_TABLE = { 14, 17, 11, 24, 1, 5, 3, 28, 15, 6, 21, 10, 23, 19, 12, 4, 26, 8, 16, 7,
			27, 20, 13, 2, 41, 52, 31, 37, 47, 55, 30, 40, 51, 45, 33, 48, 44, 49, 39, 56, 34, 53, 46, 42, 50, 36, 29,
			32 };

	private final static byte[] IP_TABLE = { 58, 50, 42, 34, 26, 18, 10, 2, 60, 52, 44, 36, 28, 20, 12, 4, 62, 54, 46,
	...
	private static final byte[] E_TABLE = { 32, 1, 2, 3, 4, 5, 4, 5, 6, 7, 8, 9, 8, 9, 10, 11, 12, 13, 12, 13, 14, 15,
			16, 17, 16, 17, 18, 19, 20, 21, 20, 21, 22, 23, 24, 25, 24, 25, 26, 27, 28, 29, 28, 29, 30, 31, 32, 1 };

	// 32-bit permutation function P used on the output of the S-boxes
	private static final byte[] P_TABLE = { 16, 7, 20, 21, 29, 12, 28, 17, 1, 15, 23, 26, 5, 18, 31, 10, 2, 8, 24, 14,
			32, 27, 3, 9, 19, 13, 30, 6, 22, 11, 4, 25 };

	private final static byte[] S_BOXSINGLE = {
			// S1
			14, 4, 13, 1, 2, 15, 11, 8, 3, 10, 6, 12, 5, 9, 0, 7, 0, 15, 7, 4, 14, 2, 13, 1, 10, 6, 12, 11, 9, 5, 3, 8,
			4, 1, 14, 8, 13, 6, 2, 11, 15, 12, 9, 7, 3, 10, 5, 0, 15, 12, 8, 2, 4, 9, 1, 7, 5, 11,
			3,
			14,
			10,
			0,
			...
			0, 14, 7, 11, 10, 4, 13, 1, 5, 8, 12, 6, 9, 3, 2, 15, 13, 8, 10, 1, 3, 15, 4, 2, 11, 6, 7,
			12,
			0,
			5,
			14,
			...
			13, 6, 4, 9, 8, 15, 3, 0, 11, 1, 2, 12, 5, 10, 14, 7, 1, 10, 13, 0, 6, 9, 8, 7, 4, 15, 14, 3,
			11,
			5,
			2,
			12,
			...
			10, 6, 9, 0, 12, 11, 7, 13, 15, 1, 3, 14, 5, 2, 8, 4, 3, 15, 0, 6, 10, 1, 13, 8, 9, 4, 5, 11, 12,
			7,
			2,
			14,
			// S5
			2, 12, 4, 1, 7, 10, 11, 6, 8, 5, 3, 15, 13, 0, 14, 9, 14, 11, 2, 12, 4, 7, 13, 1, 5, 0, 15, 10, 3, 9, 8, 6,
			4, 2, 1, 11, 10, 13, 7, 8, 15, 9, 12, 5, 6, 3, 0, 14, 11, 8, 12, 7, 1, 14, 2, 13, 6, 15, 0, 9, 10, 4,
			5,
			3,
			// S6
	...
	private static final byte[] IPR_TABLE = { 40, 8, 48, 16, 56, 24, 64, 32, 39, 7, 47, 15, 55, 23, 63, 31, 38, 6, 46,
	 ...
	 * @return
	 */
	public static String encode(String key, String dataIn) {
	...
	private static void xor(byte[] inA, int inAIndex, byte[] inB, int inBIndex, int len) {
		for (int i = 0; i < len; i++)
			inA[inAIndex + i] ^= inB[inBIndex + i];
	}

	private static void sBox(byte[] out, int outIndex, byte[] in) {
		int OutIndex = 0;
		int InIndex = 0;
		for (int i = 0, j, k; i < 8; i++) {
			j = (in[InIndex + 0] << 1) + in[InIndex + 5];
			k = (in[InIndex + 1] << 3) + (in[InIndex + 2] << 2) + (in[InIndex + 3] << 1) + in[InIndex + 4];
			byte2Bit(out, outIndex + OutIndex, S_BOXSINGLE, i * 4 * 16 + j * 16 + k, 4);
			OutIndex += 4;
			InIndex += 6;
		}
	}

	private static void function(byte[] in, int inIndex, byte[] ki) {
	...
	private static void byte2Bit(byte[] out, int outIndex, byte[] in, int inIndex, int bits) {
		for (int i = 0; i < bits; i++) {
			out[outIndex + i] = (byte) ((in[inIndex + i / 8] >> (i % 8)) & 1);
		}
	}

	private static void bit2Byte(byte[] out, int outIndex, byte[] in, int inIndex, int bits) {
	...
	private static void transform(byte[] out, int outIndex, byte[] in, int inIndex, byte[] Table, int len) {
		for (int i = 0; i < len; i++)
			TRANSFORM_TMP[i] = in[inIndex + Table[i] - 1];
		System.arraycopy(TRANSFORM_TMP, 0, out, outIndex, len);
	}

	private static void rotateL(byte[] in, int startIndex, int len, int loop) {
	...
	private static void desRun(byte[] out, int outIndex, byte[] in, int inIndex, byte type) {
	...
	private static char[] transCode(byte[] data) {
		int len = data.length;
		...
		for (int i = 0, k = 0; i < len; i++) {
			...
			if ((tempChar >= 'a' && tempChar <= 'z') || (tempChar >= 'A' && tempChar <= 'Z')
				...
				buffer[k++] = '%';
				index++;
				buffer[k++] = HEXCODE[(tempChar >>> 4) & 15]; // >>>
				index++;
				buffer[k++] = HEXCODE[tempChar & 15];
				index++;
			}
		}
	...
	private static byte[] deTransCode(byte[] data) {
		int len = data.length;

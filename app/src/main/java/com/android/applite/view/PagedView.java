...
//import com.applite.animation.ValueAnimator;
//import com.applite.animation.ValueAnimator.AnimatorUpdateListener;

/**
 * An abstraction of the original Workspace which supports browsing through a
 * sequential list of "pages" 实现翻页操作的抽象父类，有翻页效果的类都需要继承自它;
 */
public abstract class PagedView extends ViewGroup{
    private static final String TAG = "PagedView";
    private static final boolean DEBUG = false;
    protected static final int INVALID_PAGE = /*-1*/-1000;  //支持循环后有-1页，所以修改
    ...
    private static final int MINIMUM_SNAP_VELOCITY = 2200;  //初始值2200
    ...
    protected int mSnapVelocity = 500;  /*500*/  //速度
    ...
    protected boolean mIsCycle = false;

    // Scrolling indicator
//    private ValueAnimator mScrollIndicatorAnimator;
    private AlphaAnimation mScrollIndicatorAnimator;
    private ScrollIndicatorImageView mScrollIndicator;
...
//             		|Paint.ANTI_ALIAS_FLAG 
    ...
    public void scrollBy(int x, int y) {
        scrollToImpl(mUnboundedScrollX + x, getScrollY() + y);
    }

    protected void scrollToImpl(int x, int y) {
        mUnboundedScrollX = x;
    ...
    protected boolean computeScrollHelper() {
        if (mScroller.computeScrollOffset()) {
            ...
            if (getScrollX() != mScroller.getCurrX() || getScrollY() != mScroller.getCurrY()) {
                scrollToImpl(mScroller.getCurrX(), mScroller.getCurrY());
            }

            invalidate();
            return true;
        } else if (mNextPage != INVALID_PAGE) {
			if (mNextPage == -1 && mIsCycle) {
				mCurrentPage = getChildCount() - 1;
				scrollTo(getChildOffset(mCurrentPage) - getRelativeChildOffset(mCurrentPage), getScrollY());
			} else if (mNextPage == getChildCount() && mIsCycle) {
				mCurrentPage = 0;
				scrollTo(getChildOffset(mCurrentPage) - getRelativeChildOffset(mCurrentPage), getScrollY());
			} else {
...
//            }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
         ...
         * of the All apps com.applite.view on XLarge displays to not take up more space then it needs. Width
        ...
        for (int i = 0; i < childCount; i++) {
            // disallowing padding in paged com.applite.view (just pass 0)
...
//    }
    ...
    protected int getChildOffset(int index) {
        ...
        //if (mIsCycle){
	        if (index == -1){
	        	return -(getScaledMeasuredWidth(getPageAt(getChildCount()-1)) + mPageSpacing);
	        }else if (index == getChildCount()){
	        	return (getScaledMeasuredWidth(getPageAt(0)) + mPageSpacing)*index;
	        }
        //}
        ...
        if (pageCount > 0) {
            ...
            if (mIsCycle){
            	if (mOverScrollX < 0){
                    ...
                    canvas.clipRect(getScrollX(), getScrollY(), getScrollX() + getRight() - getLeft(),
                            getScrollY() + getBottom() - getTop());
        			canvas.translate(-getChildCount()*getChildWidth(0), 0);
        			drawChild(canvas, getPageAt(getChildCount()-1), getDrawingTime());
                    ...
                    canvas.clipRect(getScrollX(), getScrollY(), getScrollX() + getRight() - getLeft(),
                            getScrollY() + getBottom() - getTop());
        			canvas.translate(getChildCount()*getChildWidth(0), 0);
        			drawChild(canvas, getPageAt(0), getDrawingTime());
        			canvas.restore();
            	}
            }
        }
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (action & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_DOWN: {
                ...
                final boolean finishedScrolling = (mScroller.isFinished()/* || xDist < mTouchSlop*/);  //bug4332
    ...
    protected void animateClickFeedback(View v, final Runnable r) {
        // animate the com.applite.view slightly to show click feedback running some logic after it is "pressed"
    ...
    protected void onTouchActionUp(MotionEvent ev){
        if (mTouchState == TOUCH_STATE_SCROLLING) {
            ...
            if (((isSignificantMove && deltaX > 0 && !isFling) ||
                    (isFling && velocityX > 0)) && (mCurrentPage > 0 || mIsCycle)) {
                finalPage = returnToOriginalPage ? mCurrentPage : mCurrentPage - 1;
                snapToPageWithVelocity(finalPage, velocityX);
            } else if (((isSignificantMove && deltaX < 0 && !isFling) ||
                    (isFling && velocityX < 0)) &&
                    (mCurrentPage < getChildCount() - 1 || mIsCycle)) {
                finalPage = returnToOriginalPage ? mCurrentPage : mCurrentPage + 1;
                snapToPageWithVelocity(finalPage, velocityX);
            } else {
                snapToDestination();
...
//    }
    ...
    // We want the duration of the page snap com.applite.animation to be influenced by the distance that
    ...
    protected void snapToPageWithVelocity(int whichPage, int velocity) {
        whichPage = Math.max((mIsCycle ? -1 : 0), Math.min(whichPage, getPageCount() -  (mIsCycle ? 0 : 1)));
        int halfScreenSize = getMeasuredWidth() / 2;
        ...
        velocity = Math.min(MINIMUM_SNAP_VELOCITY,Math.max(MINIMUM_SNAP_VELOCITY, velocity));
        ...
        if (DEBUG) Log.d(TAG, "snapToPageWithVelocity::velocity=" + velocity+",duration="+duration);
...
//            mActionMode = startActionMode(callback);
    ...
    /**
     * This method is called ONLY to synchronize the number of pages that the paged com.applite.view has.
    ...
    protected ImageView getScrollingIndicator() {
        ...
        if (mHasScrollIndicator && mScrollIndicator == null) {
            ...
            mScrollIndicator = (ScrollIndicatorImageView) (parent.findViewById(R.id.paged_view_indicator));
    ...
    protected boolean isScrollingIndicatorEnabled() {
        return !AppLiteApplication.isScreenLarge();
    }
    ...
    protected void showScrollingIndicator(boolean immediately) {
        ...
        if (mScrollIndicator != null) {
            ...
            if (immediately) {
                mScrollIndicator.setAlpha(255);
            } else {
            	mScrollIndicatorAnimator=new AlphaAnimation(0, 1);
//                mScrollIndicatorAnimator = ValueAnimator.ofInt(0,255);
//                mScrollIndicatorAnimator.addUpdateListener(new AnimatorUpdateListener() {
//					@Override
//					public void onAnimationUpdate(ValueAnimator animation) {
//						// TODO Auto-generated method stub
//						int alpha = (Integer) animation.getAnimatedValue();
//						mScrollIndicator.setAlpha(alpha);
//					}
//				});
                mScrollIndicatorAnimator.setDuration(sScrollIndicatorFadeInDuration);
//                mScrollIndicatorAnimator.start();
                mScrollIndicator.startAnimation(mScrollIndicatorAnimator);
            }
        }
    }
    ...
    protected void hideScrollingIndicator(boolean immediately) {
        ...
        if (mScrollIndicator != null) {
            ...
            if (immediately) {
                ...
                mScrollIndicator.setAlpha(0);
            } else {
            	mScrollIndicatorAnimator=new AlphaAnimation(1, 0);
//                mScrollIndicatorAnimator = ValueAnimator.ofInt(255,0);
//                mScrollIndicatorAnimator.addUpdateListener(new AnimatorUpdateListener() {
//					@Override
//					public void onAnimationUpdate(ValueAnimator animation) {
//						// TODO Auto-generated method stub
...
//				});
                ...
                mScrollIndicator.startAnimation(mScrollIndicatorAnimator);
                mScrollIndicatorAnimator.setAnimationListener(new AnimationListener() {
...
//                    private boolean cancelled = false;
//                    @Override
//                    public void onAnimationCancel(com.applite.animation.Animator animation) {
//                        cancelled = true;
//                    }
//                    @Override
...
//                mScrollIndicatorAnimator.start();
    ...
    private void updateScrollingIndicatorPosition() {
        ...
        float offset = (float) getScrollX() / maxScrollX;//Math.max(0f, Math.min(1f, (float) getScrollX() / maxScrollX));
...
//        mScrollIndicator.offsetLeftAndRight(indicatorPos-mScrollIndicator.getLeft());

...
public class SimpleWeekView extends View {
    ...
    protected static int DAY_SEPARATOR_WIDTH = 3;//1

    protected static int MINI_DAY_NUMBER_TEXT_SIZE = 20;
    protected static int MINI_WK_NUMBER_TEXT_SIZE = 12;
    protected static int MINI_TODAY_NUMBER_TEXT_SIZE = 24;
    ...
    protected int[] mYearNumbers;
    protected int[] mMonthNumbers;
    private int position;
    protected boolean[] mUnWorkDay;
    private static String FIRSTDAY = "初一";
    ...
    public SimpleWeekView(Context context) {
        ...
        mBGColor = res.getColor(R.color.theme);
    ...
    public void setWeekParams(HashMap<String, Integer> params, String tz) {
        ...
        mDayNumbers = new int[mNumCells];
        mYearNumbers = new int[mNumCells];
        mMonthNumbers = new int[mNumCells];
        mFocusDay = new boolean[mNumCells];
        mOddMonth = new boolean[mNumCells];
        mUnWorkDay = new boolean[mNumCells];
        ...
        int i = 0;
        if (mShowWeekNum) {
            mDayNumbers[0] = time.getWeekNumber();
            i++;
        }
        ...
        for (; i < mNumCells; i++) {
            ...
            if (focusMonth == DEFAULT_FOCUS_MONTH) {
                mFocusDay[i] = true;
            } else {
            ...
            mMonthNumbers[i] = time.month;
            mYearNumbers[i] = time.year;
            mDayNumbers[i] = time.monthDay++;
            time.normalize(true);
        }
    ...
    public Time getDayFromLocation(float x) {
        ...
        Utils.mFocusDay = mFocusDay[dayPosition];
...
//        drawBackground(canvas);
    ...
    protected void drawWeekNums(Canvas canvas) {
        int y = ((mHeight + MINI_DAY_NUMBER_TEXT_SIZE) / 2) - DAY_SEPARATOR_WIDTH * 3;
        int nDays = mNumCells;
        ...
        if (mShowWeekNum) {
            ...
            canvas.drawText(Integer.toString(mDayNumbers[0]), x, y, p);
            i++;
        }
...
//            	mMonthNumPaint.setColor(Color.BLUE);
            ...
            //ondraw today
            if (mHasToday && mToday == i && mFocusDay[i]) {
                mMonthNumPaint.setTextSize(Math.round(mHeight / (2.5)));
                mMonthNumPaint.setColor(getResources().getColor(R.color.theme));
//                mMonthNumPaint.setColor(isFocusMonth ? mFocusMonthColor : mOtherMonthColor);
                mMonthNumPaint.setFakeBoldText(true);
            }
            int x = (2 * i + 1) * (mWidth - mPadding * 2) / (divisor) + mPadding;
            int mMonSelectedDay = mSelectedDay;
            ...
            if (mHasSelectedDay && mMonSelectedDay == i) {
                //ondraw selected day
                mMonthNumPaint.setTextSize(Math.round(mHeight / (2)));
                mMonthNumPaint.setColor(Color.WHITE);
                canvas.drawText(Integer.toString(mDayNumbers[i]), x, y + DAY_SEPARATOR_WIDTH * 2, mMonthNumPaint);
            } else {
                //ondraw unselected day
                mMonthNumPaint.setTextSize(Math.round(mHeight / (2.5)));
                canvas.drawText(Integer.toString(mDayNumbers[i]), x, y, mMonthNumPaint);
            }
            LunarCalendarConvertUtil.parseLunarCalendar(mYearNumbers[i], mMonthNumbers[i], mDayNumbers[i], lc);
            String[] detailInfo = lc.getLunarCalendarInfo(false);
            String dayString = detailInfo[2];
            if (FIRSTDAY.equals(detailInfo[2])) {
                dayString = detailInfo[1];
            }
            if (detailInfo[4].length() != 0) {
                // add festival
                dayString = detailInfo[4];
            }
            if (detailInfo[5].length() != 0) {
                // add solar term
                dayString = detailInfo[5];
            }
            if (detailInfo[3].length() != 0) {
                // add traditional festival
                dayString = detailInfo[3];
            }
//            mMonthNumPaint.setTextSize(15);
            mMonthNumPaint.setColor(mOtherMonthColor);
            if (mHasToday && mToday == i && mFocusDay[i]) {
//                mMonthNumPaint.setTextSize(MINI_DAY_NUMBER_TEXT_SIZE);
                mMonthNumPaint.setColor(getResources().getColor(R.color.theme));
            }
            //ondraw Lunar date(被选中的没有农历)
            float height = 0;
            if (mHasSelectedDay && mMonSelectedDay == i) {

            } else {
                mMonthNumPaint.setTextSize(Math.round(mHeight / (2.5)));
                mFontMetrics = mMonthNumPaint.getFontMetrics();
                float fontHeight = mFontMetrics.bottom - mFontMetrics.top;
                if (dayString.length() >= 4) {
                    mMonthNumPaint.setTextSize(Math.round(mHeight / (5.6)));
                } else {
                    mMonthNumPaint.setTextSize(Math.round(mHeight / (5.25)));
                }
                float b = (float) Math.floor(fontHeight / 2);
                height = (float) (b - 1.5 + y);
                canvas.drawText(dayString, x, height, mMonthNumPaint);

            }
            if (mEventDate != null/*&&isFocusMonth*/) {
                for (int k = 0; k < mEventDate.size(); k++) {
                    String date = mEventDate.get(k);
                    Calendar calendar = Utils.strToCalendar(date);
                    if (calendar.get(Calendar.YEAR) == mYearNumbers[i] && calendar.get(Calendar.MONTH) == mMonthNumbers[i] && calendar.get(Calendar.DAY_OF_MONTH) == mDayNumbers[i]) {
                        mMonthNumPaint.setAntiAlias(true);
                        mMonthNumPaint.setTextSize(Math.round(mHeight / (5.27)));
                        mFontMetrics = mMonthNumPaint.getFontMetrics();
                        float fontHeight = mFontMetrics.bottom - mFontMetrics.top;
                        float b = (float) (Math.floor(fontHeight / 2) - 0.5);
                        if (mHasSelectedDay && mMonSelectedDay == i) {
                            float cy = getHeight() / 2 + (mSelectedRight - 1 - (mSelectedLeft + 1)) / 2 - 5 * 2 - 2;// 5*2直径
                            mMonthNumPaint.setColor(Color.WHITE);
                            canvas.drawCircle(x, cy, 5.0f, mMonthNumPaint);
                        } else {
                            mMonthNumPaint.setColor(getResources().getColor(R.color.theme));
                            canvas.drawCircle(x, b + height, 5.0f, mMonthNumPaint);
                        }
                        break;
                    }
                }
    ...
    protected void drawDaySeparators(Canvas canvas) {
        if (mHasSelectedDay) {
            ...
            p.setColor(mBGColor);
//            canvas.drawRect(r, p);
            int cx = (mSelectedRight - 1 - (mSelectedLeft + 1)) / 2 + mSelectedLeft + 1;
            int cy = (mHeight - 1 - 1) / 2 - DAY_SEPARATOR_WIDTH;
            canvas.drawCircle(cx, cy + 1, (mSelectedRight - 1 - (mSelectedLeft + 1)) / 2, p);
        }
        if (mShowWeekNum) {
    ...
    public boolean isOnClickSelf() {
        return isOnClick;
    }

    public void setOnClickSelf(boolean isOnClick) {
        this.isOnClick = isOnClick;
    }

    public void setEventDate(List<String> mEventDate) {
